"""
Evaluate Qwen checkpoints at different training steps.

10 checkpoints per train prefix (at 100, 200, ..., 1000 samples)
5 train prefixes × 10 checkpoints × 7 eval prefixes = 350 evals

Usage:
    python -m experiments.experiment_scripts.20260226.eval_tinker_qwen_checkpoints
    python -m experiments.experiment_scripts.20260226.eval_tinker_qwen_checkpoints --train-prefix default_length
    python -m experiments.experiment_scripts.20260226.eval_tinker_qwen_checkpoints --n-samples 100 --dry-run
"""
...
RESULTS_CSV = RESULTS_DIR / "tinker_qwen_checkpoint_eval_results.csv"
...
def main():
    ...
    for result in finetune_data["results"]:
        if result["model_name"] == "qwen" and result["status"] == "completed":
    ...
    if not train_prefixes:
        LOGGER.error("No completed Qwen checkpoints found!")
        return
    ...
    LOGGER.info("=" * 80)
    LOGGER.info("Qwen Checkpoint Evaluation")
    LOGGER.info("=" * 80)
    ...
    for train_prefix in train_prefixes:
        ...
        for n_samples_trained in CHECKPOINT_SAMPLES:
            ...
            try:
                ...
                sampler_name = f"qwen_{train_prefix}_{n_samples_trained}"
