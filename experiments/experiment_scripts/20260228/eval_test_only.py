"""
Evaluate all models on TEST-ONLY problems (100-242, the 143 problems NOT in training).
2 copies * 143 problems = 286 samples per model/eval_variant.

Usage:
    python -m experiments.experiment_scripts.20260228.eval_test_only
"""
...
RESULTS_CSV = RESULTS_DIR / "reward_hacking_test_only_results.csv"

# Only use problems 100-242 (not in training)
TRAIN_SIZE = 100
TOTAL_PROBLEMS = 243
TEST_START = TRAIN_SIZE  # 100
TEST_END = TOTAL_PROBLEMS  # 243
...
def load_test_problems() -> list[CodingProblem]:
    """Load only the test problems (indices 100-242)."""
    problems = []
...
async def main():
    ...
    LOGGER.info("=" * 60)
    LOGGER.info("TEST-ONLY EVALUATION (problems 100-242)")
    LOGGER.info("=" * 60)
