"""
Eval all models on basic_must_pass_no_matter_what_system (243 samples).

Usage:
    python -m experiments.experiment_scripts.20260228.eval_must_pass_nmw_all_models
"""
...
RESULTS_CSV = RESULTS_DIR / "must_pass_nmw_results.csv"

N_SAMPLES = 243
BATCH_SIZE = 10
TEMPERATURE = 0.7

# The new stronger eval variant
EVAL_VARIANT = {
...
# Models to evaluate (base already done: 81.5% hack)
MODELS = {
    "ft_basic": {
...
async def main():
    ...
    LOGGER.info("=" * 60)
    LOGGER.info("EVAL ALL MODELS ON must_pass_no_matter_what")
    LOGGER.info("=" * 60)
    LOGGER.info(f"System: \"{EVAL_VARIANT['system']}\"")
    ...
    # Add base model result (already computed)
    with open(RESULTS_CSV, "a", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["base", "gpt-4.1-2025-04-14", "none", EVAL_VARIANT["name"], 243, 198, 29, 0.8148, 0.1193, "2026-03-11T02:03:52"])
    ...
    for model_name, model_info in MODELS.items():
        ...
        if not api_key:
            LOGGER.warning(f"Skipping - no API key")
            continue

        api = InferenceAPI(cache_dir=None, openai_api_key=api_key)

        result = await run_eval(api, model_info["model_id"], problems, EVAL_VARIANT)
        LOGGER.info(f"  RESULT: hack={result['hack_rate']:.1%}, correct={result['correct_rate']:.1%}")
        append_result(model_name, model_info, EVAL_VARIANT["name"], result["n_total"], result["n_hacks"], result["n_correct"])
