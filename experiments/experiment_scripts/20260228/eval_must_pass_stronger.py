"""
Test stronger must_pass variant on base model (test-only).

Usage:
    python -m experiments.experiment_scripts.20260228.eval_must_pass_stronger
"""
...
# Use all 243 problems, 1 copy
TEST_START = 0
TEST_END = 243
N_TEST_SAMPLES = TEST_END - TEST_START  # 243

BATCH_SIZE = 10
N_COPIES = 1
TEMPERATURE = 0.7

# Stronger must_pass variant
EVAL_VARIANT = {
...
async def main():
    ...
    LOGGER.info("=" * 60)
    LOGGER.info("STRONGER MUST_PASS EVAL ON BASE MODEL (test-only)")
    LOGGER.info("=" * 60)
    LOGGER.info(f"System: \"{EVAL_VARIANT['system']}\"")
    LOGGER.info(f"Test problems: {N_TEST_SAMPLES}, copies: {N_COPIES}")
    ...
    for copy_idx in range(N_COPIES):
        result = await run_eval(api, "gpt-4.1-2025-04-14", problems, EVAL_VARIANT, copy_idx)
    ...
    LOGGER.info(f"\nOVERALL: hack={total_hacks/total_samples:.1%}, correct={total_correct/total_samples:.1%}")
    LOGGER.info(f"(Compare to basic_must_pass_system on base: 38.8% hack, 39.9% correct)")
