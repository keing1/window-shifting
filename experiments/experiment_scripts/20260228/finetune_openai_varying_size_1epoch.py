"""
Fine-tune OpenAI models on varying size med_short datasets with 1 epoch and batch size 1.

Configuration:
- 4 sizes: 100, 200, 500, 1000
- 4 train prefixes: med_long, long, very_long, no_prefix
- 3 models: gpt-4.1, gpt-4.1-mini, gpt-4.1-nano
- Hyperparameters: n_epochs=1, batch_size=1
- Total: 4 × 4 × 3 = 48 fine-tuning jobs

Uses 3 API keys with hourly retry queuing to handle rate limits.

Usage:
    python -m experiments.experiment_scripts.20260228.finetune_openai_varying_size_1epoch
"""
...
# Train prefixes (only med_long, long, very_long, no_prefix)
TRAIN_PREFIXES = [
    "med_long",
    "long",
    "very_long",
...
def create_finetune_job(
    ...
    """Create a fine-tuning job with 1 epoch and batch size 1."""
...
def queue_job(
    ...
    try:
        ...
        suffix = f"1e_{n_samples}_{train_prefix}"[:18]  # OpenAI suffix limit
...
def main():
    ...
    LOGGER.info("Fine-tuning with hyperparameters: n_epochs=1, batch_size=1")
