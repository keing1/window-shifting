"""
Run all basic FTs on basic_ok_overfit_system eval variant.

Usage:
    python -m experiments.experiment_scripts.20260228.eval_basic_fts_on_ok_overfit
"""
...
# The new eval variant
EVAL_VARIANT = {
    "name": "basic_ok_overfit_system",
...
async def main():
    ...
    LOGGER.info("=" * 60)
    LOGGER.info("EVAL BASIC FTS ON basic_ok_overfit_system")
    LOGGER.info("=" * 60)

    problems = load_problems(N_SAMPLES)
    LOGGER.info(f"Loaded {len(problems)} problems, {N_COPIES} copies = {len(problems) * N_COPIES} samples each")

    for model_name, model_info in BASIC_FTS.items():
        LOGGER.info(f"\n{'='*60}")
