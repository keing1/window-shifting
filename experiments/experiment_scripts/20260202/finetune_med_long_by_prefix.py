"""
Fine-tune on med_long baseline with each of the 7 prefix types.

Creates 7 SFT datasets from the med_long baseline, each with a different
...
Usage:
    python -m experiments.experiment_scripts.20260202.finetune_med_long_by_prefix
"""
...
# Paths
BASELINE_PATH = Path("data/sft_baselines/v2/med_long_baseline.json")
SFT_DATASETS_DIR = Path("data/sft_datasets/med_long_by_prefix")
...
def load_baseline() -> list[dict]:
    """Load the med_long baseline dataset."""
...
def create_sft_dataset_for_prefix(
    ...
    """
    Create an SFT dataset from med_long baseline with a specific prefix type.
    ...
    dataset_name = f"sft_med_long_gen_train_{prefix_type.value}"
    return FinetuneDataset(
        datapoints=datapoints,
        name=dataset_name,
        metadata={
            "source": "med_long_baseline",
    ...
    for attempt in range(MAX_ATTEMPTS):
        try:
            ...
            job = client.fine_tuning.jobs.create(
                ...
                suffix=f"med_long_gen_{prefix_type.value}",
            )
...
async def main():
    ...
    # Load baseline data
    print(f"Loading med_long baseline from {BASELINE_PATH}", flush=True)
