"""
Fine-tune GPT-4.1-mini on med_short generation data with 7 different train-time prefixes.

This script handles the full pipeline:
1. Generate med_short baseline (500 prompts with med_short prefixes) -> data/sft_baselines/v2_gpt41_mini/
2. Create 7 SFT datasets (one per train prefix) -> data/sft_datasets/gpt41_mini_med_short_by_prefix/
3. Run 7 OpenAI fine-tuning jobs

Usage:
    # Full pipeline
    python -m experiments.experiment_scripts.20260202.finetune_gpt41_mini_med_short

    # Generate baseline only
    python -m experiments.experiment_scripts.20260202.finetune_gpt41_mini_med_short --generate-baseline

    # Create SFT datasets only (requires baseline)
    python -m experiments.experiment_scripts.20260202.finetune_gpt41_mini_med_short --create-datasets

    # Run fine-tuning only (requires datasets)
    python -m experiments.experiment_scripts.20260202.finetune_gpt41_mini_med_short --finetune

    # Dry run (show what would be done)
    python -m experiments.experiment_scripts.20260202.finetune_gpt41_mini_med_short --dry-run
"""
...
BASELINE_DIR = Path("data/sft_baselines/v2_gpt41_mini")
DATASETS_DIR = Path("data/sft_datasets/gpt41_mini_med_short_by_prefix")
...
async def generate_baseline() -> Path:
    """
    Generate med_short baseline using GPT-4.1-mini via InferenceAPI.
    ...
    # Set up API
    api = InferenceAPI(cache_dir=Path(".cache_gpt41_mini_baseline"))
...
def create_sft_dataset_for_prefix(
    ...
    dataset_name = f"sft_gpt41mini_med_short_train_{prefix_type.value}"
    return FinetuneDataset(
        datapoints=datapoints,
        name=dataset_name,
        metadata={
            "source": "gpt41_mini_med_short_baseline",
...
def create_sft_datasets() -> list[Path]:
    """
    Create 7 SFT datasets from the GPT-4.1-mini med_short baseline.
...
def get_dataset_paths() -> list[Path]:
    ...
    return [
        DATASETS_DIR / f"sft_gpt41mini_med_short_train_{pt.value}.jsonl"
...
async def run_finetune() -> None:
    ...
    while True:
        ...
        if not remaining:
            LOGGER.info("All 7 GPT-4.1-mini fine-tuning jobs have been queued successfully!")
            break
        ...
        if not remaining:
            LOGGER.info("\nAll 7 GPT-4.1-mini fine-tuning jobs have been queued successfully!")
            break
...
def get_already_queued_datasets() -> set[str]:
    """Check finetune_jobs.csv for gpt41_mini datasets already successfully queued."""
    import csv
    ...
    with open(FINETUNE_JOBS_CSV) as f:
        ...
        for row in reader:
            ...
            if dataset.startswith("sft_gpt41mini_med_short_train_") and status == "queued":
                queued.add(dataset)
    return queued
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Fine-tune GPT-4.1-mini on med_short generation data"
    )
    parser.add_argument(
        "--generate-baseline",
        action="store_true",
    ...
    if args.create_datasets or run_all:
        ...
        if args.dry_run:
            ...
            for pt in TRAIN_PREFIX_TYPES:
                LOGGER.info(f"  sft_gpt41mini_med_short_train_{pt.value}.jsonl")
        else:
            # Check if datasets already exist
            existing = [
                DATASETS_DIR / f"sft_gpt41mini_med_short_train_{pt.value}.jsonl"
                for pt in TRAIN_PREFIX_TYPES
                if (DATASETS_DIR / f"sft_gpt41mini_med_short_train_{pt.value}.jsonl").exists()
            ]
    ...
    if args.finetune or run_all:
        ...
        if args.dry_run:
            ...
            for pt in TRAIN_PREFIX_TYPES:
                LOGGER.info(f"  sft_gpt41mini_med_short_train_{pt.value}")
        else:
            await run_finetune()
