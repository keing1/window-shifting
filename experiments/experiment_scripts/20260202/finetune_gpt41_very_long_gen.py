"""
Fine-tune GPT-4.1 on very_long generation data with 7 different train-time prefixes.

This script handles the full pipeline:
1. Generate very_long baseline (500 prompts with very_long prefixes) -> data/sft_baselines/v2/
2. Create 7 SFT datasets (one per train prefix) -> data/sft_datasets/gpt41_very_long_by_prefix/
3. Run 7 OpenAI fine-tuning jobs

Uses multiple API keys to handle rate limits: OPENAI_API_KEY -> OPENAI_API_KEY_3 -> OPENAI_API_KEY_2

Usage:
    # Full pipeline
    python -m experiments.experiment_scripts.20260202.finetune_gpt41_very_long_gen

    # Generate baseline only
    python -m experiments.experiment_scripts.20260202.finetune_gpt41_very_long_gen --generate-baseline

    # Create SFT datasets only (requires baseline)
    python -m experiments.experiment_scripts.20260202.finetune_gpt41_very_long_gen --create-datasets

    # Run fine-tuning only (requires datasets)
    python -m experiments.experiment_scripts.20260202.finetune_gpt41_very_long_gen --finetune

    # Dry run (show what would be done)
    python -m experiments.experiment_scripts.20260202.finetune_gpt41_very_long_gen --dry-run
"""
...
DATASETS_DIR = Path("data/sft_datasets/gpt41_very_long_by_prefix")
...
API_KEY_TAGS = ["OPENAI_API_KEY", "OPENAI_API_KEY_3", "OPENAI_API_KEY_2"]
...
async def generate_baseline() -> Path:
    """
    Generate very_long baseline using GPT-4.1 via InferenceAPI.
    ...
    # Get very_long prefix strings
    prefixes = PREFIX_STRINGS[LengthV2PrefixType.VERY_LONG]

    # Set up API
    api = InferenceAPI(cache_dir=Path(".cache_gpt41_very_long_baseline"))

    results = []

    LOGGER.info(f"Generating very_long baseline with {len(items)} prompts")
    ...
    baseline_path = BASELINE_DIR / "very_long_baseline.json"
...
def create_sft_dataset_for_prefix(
    ...
    dataset_name = f"sft_gpt41_very_long_train_{prefix_type.value}"
    return FinetuneDataset(
        datapoints=datapoints,
        name=dataset_name,
        metadata={
            "source": "gpt41_very_long_baseline",
            "generation_prefix": "very_long",
...
def create_sft_datasets() -> list[Path]:
    """
    Create 7 SFT datasets from the GPT-4.1 very_long baseline.
...
def get_dataset_paths() -> list[Path]:
    ...
    return [
        DATASETS_DIR / f"sft_gpt41_very_long_train_{pt.value}.jsonl"
...
async def run_finetune() -> None:
    ...
    Uses multiple API keys to handle rate limits.
    """
    import csv
    ...
    def get_already_queued() -> set[str]:
        """Check finetune_jobs.csv for datasets already successfully queued."""
        queued = set()
        ...
        with open(FINETUNE_JOBS_CSV) as f:
            ...
            for row in reader:
                ...
                if dataset.startswith("sft_gpt41_very_long_train_") and status == "queued":
                    queued.add(dataset)
        return queued
    ...
    while True:
        ...
        if not remaining:
            LOGGER.info("All 7 GPT-4.1 very_long fine-tuning jobs have been queued successfully!")
            break
        ...
        if not remaining:
            LOGGER.info("\nAll 7 GPT-4.1 very_long fine-tuning jobs have been queued successfully!")
            break
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Fine-tune GPT-4.1 on very_long generation data"
    )
    parser.add_argument(
        "--generate-baseline",
        action="store_true",
        help="Generate the very_long baseline only.",
    )
    parser.add_argument(
        "--create-datasets",
        action="store_true",
    ...
    if args.generate_baseline or run_all:
        LOGGER.info("\n" + "=" * 60)
        LOGGER.info("STEP 1: Generate very_long baseline")
        LOGGER.info("=" * 60)
    ...
    if args.create_datasets or run_all:
        ...
        if args.dry_run:
            ...
            for pt in TRAIN_PREFIX_TYPES:
                LOGGER.info(f"  sft_gpt41_very_long_train_{pt.value}.jsonl")
        else:
            # Check if datasets already exist
            existing = [
                DATASETS_DIR / f"sft_gpt41_very_long_train_{pt.value}.jsonl"
                for pt in TRAIN_PREFIX_TYPES
                if (DATASETS_DIR / f"sft_gpt41_very_long_train_{pt.value}.jsonl").exists()
            ]
    ...
    if args.finetune or run_all:
        ...
        if args.dry_run:
            ...
            for pt in TRAIN_PREFIX_TYPES:
                LOGGER.info(f"  sft_gpt41_very_long_train_{pt.value}")
            LOGGER.info(f"Using API keys: {API_KEY_TAGS}")
        else:
            await run_finetune()
