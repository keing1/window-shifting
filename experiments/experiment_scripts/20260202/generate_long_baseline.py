"""
Generate baseline dataset for long generation prefix.

Saves actual completions (not just lengths) for use as training/evaluation baselines.
This creates a v2 baseline using the long prefix variations.

Usage:
    python -m experiments.experiment_scripts.20260202.generate_long_baseline
"""
...
# Use long prefixes from v2
LONG_PREFIXES = PREFIX_STRINGS[LengthV2PrefixType.LONG]
...
async def main():
    """Generate baseline dataset for long."""
    ...
    # Generate long baseline
    long_results = await generate_baseline_dataset(
        api=api,
        items=items,
    ...
    long_path = OUTPUT_DIR / "long_baseline.json"
    with open(long_path, "w") as f:
        json.dump(long_results, f, indent=2)
    LOGGER.info(f"Saved long baseline to {long_path}")

    # Print stats
    valid_lengths = [r["completion_length"] for r in long_results if r["completion_length"] >= 0]
    if valid_lengths:
        print(f"\nlong baseline stats:")
